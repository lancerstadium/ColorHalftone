...
void TinyLUT_init(TinyLUT *mdl, char* dir, char* modes, int nmode, int nstage, int bitwidth, int interval, float scale) {
    ...
    for(stage = 1; stage <= nstage; stage++) {
        mode = modes[stage - 1];
        lidx = (stage - 1) * 2;
        sprintf(lut_path, "%s/x%d_%db_i%d_s%d_%c_L2.bin", dir, (int)scale, interval, bitwidth, stage, mode);
        mdl->lut[lidx] = TensorData_load(lut_path);
        printf("Load lut[%d]<%s>(%d, %d): %s\n", lidx, TensorType_str(mdl->lut[lidx]->dtype), mdl->lut[lidx]->shape[0], mdl->lut[lidx]->shape[1], lut_path);
        sprintf(lut_path, "%s/x%d_%db_i%d_s%d_%c_H6.bin", dir, (int)scale, interval, bitwidth, stage, mode);
...
void TinyLUT_var_init(TinyLUT *mdl, char* dir, char* modes, int nmode, int nstage, int bitwidth, int interval, float scale) {
    ...
    for(stage = 1; stage <= nstage; stage++) {
        mode = modes[stage - 1];
        lidx = (stage - 1) * 2;
        sprintf(lut_path, "%s/x4.00_%db_i%d_s%d_%c_L2.bin", dir, interval, bitwidth, stage, mode);
        mdl->lut[lidx] = TensorData_load(lut_path);
        printf("Load lut[%d]<%s>(%d, %d): %s\n", lidx, TensorType_str(mdl->lut[lidx]->dtype), mdl->lut[lidx]->shape[0], mdl->lut[lidx]->shape[1], lut_path);
        sprintf(lut_path, "%s/x4.00_%db_i%d_s%d_%c_H6.bin", dir, interval, bitwidth, stage, mode);
        mdl->lut[lidx + 1] = TensorData_load(lut_path);
        printf("Load lut[%d]<%s>(%d, %d): %s\n", lidx + 1, TensorType_str(mdl->lut[lidx + 1]->dtype), mdl->lut[lidx + 1]->shape[0], mdl->lut[lidx + 1]->shape[1], lut_path);
#ifdef LUT_PERF
        // <<< Add LUT Size to perf_mem >>>
